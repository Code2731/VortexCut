...
│  │ (IProject-   │  │          │  │ (ClipCanvasPanel │  │
│  │  Service)    │  │          │  │  7 partial files)│  │
│  └──────┬───────┘  └──────────┘  └──────────────────┘  │
│         │                                               │
│  ┌──────▼───────────────────────────────────────────┐   │
│  │  ProjectService : IProjectService                │   │
│  │  (DI: IRenderService, TimelineService 주입)      │   │
│  └──────┬───────────────────────────────────────────┘   │
├─────────┼───────────────────────────────────────────────┤
│  ┌──────▼───────────────────────────────────────────┐   │
│  │  VortexCut.Core (모델 + 인터페이스)              │   │
│  │  IProjectService, IRenderService, IExportService │   │
│  │  IAudioPlaybackService, IRenderedFrame, VideoInfo│   │
│  └──────────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────┐   │
...
### 2.1.1 DI 컨테이너 (Composition Root)

```
App.axaml.cs
├── TimelineService              (Interop, Singleton)
...
│   ├── Models/               # ClipModel, VideoInfo, Project, SubtitleClipModel
│   └── Interfaces/           # IProjectService, IRenderService, IExportService,
│                              # IAudioPlaybackService, IRenderedFrame
├── VortexCut.Interop/        # Rust-C# P/Invoke
│   ├── NativeMethods.cs      # FFI 선언 (~50개 함수)
│   └── Services/
│       ├── RenderService.cs       # IRenderService 구현 (비디오 렌더링)
│       ├── AudioPlaybackService.cs # IAudioPlaybackService 구현
│       ├── ExportService.cs       # IExportService 구현
│       └── ThumbnailStripService.cs # 썸네일 생성
├── VortexCut.UI/             # Avalonia UI (DI + MVVM)
│   ├── ViewModels/
│   │   ├── MainViewModel.cs               # 필드, 생성자, 프로퍼티
│   │   ├── MainViewModel.FileOperations.cs # 파일 I/O 커맨드
│   │   ├── MainViewModel.ClipOperations.cs # 클립/재생/Export 커맨드
│   │   ├── PreviewViewModel.cs            # 프리뷰 (IProjectService 의존)
│   │   ├── TimelineViewModel.cs           # 타임라인 (IProjectService 의존)
│   │   ├── InspectorViewModel.cs          # 색보정/오디오/트랜지션 비즈니스 로직
│   │   └── ExportViewModel.cs             # Export (IExportService 의존)
│   ├── Controls/Timeline/
│   │   ├── ClipCanvasPanel.cs             # 필드, 프로퍼티 (~170줄)
│   │   ├── ClipCanvasPanel.Rendering.cs   # 트랙/플레이헤드/스냅 (~700줄)
│   │   ├── ClipCanvasPanel.ClipRendering.cs # 클립/썸네일/파형 (~550줄)
│   │   ├── ClipCanvasPanel.Input.cs       # 마우스 입력 (~500줄)
│   │   ├── ClipCanvasPanel.Keyframes.cs   # 키프레임/링크 (~120줄)
